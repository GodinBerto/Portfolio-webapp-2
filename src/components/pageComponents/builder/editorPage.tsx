...
export default function BuilderEditorPage({
  ...
  useEffect(() => {
    ...
    const storageValue = window.localStorage.getItem(BUILDER_PROJECTS_STORAGE_KEY);
    let projects: StoredBuilderProject[] = [];

    if (storageValue) {
      try {
        const parsed = JSON.parse(storageValue) as StoredBuilderProject[];
  ...
  const updateProjectPreview = useCallback(
    (canvas: fabric.Canvas) => {
      if (!projectId || typeof window === "undefined") return;
      ...
      if (existingProjectIndex >= 0) {
        ...
        projects[existingProjectIndex] = {
          ...existingProject,
          updatedAt: now,
          roomId: roomId ?? existingProject.roomId,
          thumbnailDataUrl:
            thumbnailDataUrl && thumbnailDataUrl.length > 0
              ? thumbnailDataUrl
              : existingProject.thumbnailDataUrl,
        };
      } else {
        projects = [
          {
  ...
  const pushHistory = useCallback(
    (canvas: fabric.Canvas) => {
      ...
      historyIndexRef.current = historyRef.current.length - 1;
      syncHistoryAvailability();
      updateProjectPreview(canvas);
      queueSnapshotBroadcast(canvas);
    },
    [queueSnapshotBroadcast, syncHistoryAvailability, updateProjectPreview]
  );
