...
const Calendar: React.FC<CalendarProps> = ({ onDateSelect, className }) => {
  ...
  if (!mounted || !currentDate) {
    return (
      <div
        className={`w-[350px] h-[400px] bg-white dark:bg-gray-800 rounded-xl shadow-lg animate-pulse`}
      />
    );
  }
    ...
    >
      ...
      <div className="flex justify-between items-center mb-6 gap-5">
        ...
        <div className="relative" ref={yearSelectRef}>
          ...
          {showYearSelect && (
            <div
              className="absolute z-10 mt-2 py-2 w-44 max-h-60 bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-xl dark:shadow-black/20 top-full left-1/2 transform -translate-x-1/2
              overflow-y-auto
            ...
            >
              {generateYearOptions().map((year) => (
                <button
                  key={year}
                  ref={
                    year === currentDate.getFullYear() ? currentYearRef : null
                  }
